...
# lines = open(train_data_file, "r").read().split("\n")
# for i, line in enumerate(lines):
#     if line.startswith("\""):
#         lines[i-1] = lines[-1].strip()  + line
#         lines.pop(i)

# open("train_new.txt", "w").write("\n".join(lines)+"\n")
...
def train_and_test(t):
    ...
    with open(train_data_file, 'r') as trainfile:
        for line in trainfile:
            ...
            if line[0].strip() != 'ID' and line[3].strip() == 'FAVOR' and line[1] == TARGETS[t]:
                tweet = line[2]
                tweet = process_tweet(tweet)
                if len(tweet) > sentence_maxlen:
                    sentence_maxlen = len(tweet)
                all_favor_tweets.append(tweet)
            elif line[0].strip() != 'ID' and line[3].strip() == 'AGAINST' and line[1] == TARGETS[t]:
                tweet = line[2]
                tweet = process_tweet(tweet)
                if len(tweet) > sentence_maxlen:
                    sentence_maxlen = len(tweet)
    ...
    with open(test_data_file, 'r') as testfile:
        for line in testfile:
            ...
            if line[0] != 'ID' and line[3] == 'FAVOR' and line[1] == TARGETS[t]:
                tweet = line[2]
                tweet = process_tweet(tweet)
                if len(tweet) > sentence_maxlen:
                    sentence_maxlen = len(tweet)
...
x_train, y_train, x_test, y_test, sentence_maxlen = train_and_test(t)
...
print("For",TARGETS[t],classification_report(y_test, y_pred, digits=4))
