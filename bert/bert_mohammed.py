...
#get_ipython().system('pip install pytorch-pretrained-bert pytorch-nlp #if you submit the job then first install it in your folder as : pip install --user pytorch-pretrained-bert pytorch-nlp')
...
def train_and_test(): # feed raw text in BERT so we will not preprocess
...
train_texts, train_labels, test_texts, test_labels, sen_maxlen, pp = train_and_test()
...
print('Batch size: {} \nLearning rate:{} \nNumber of epochs={}'.format(BATCH_SIZE, learning_rate, EPOCHS), file=open("output.txt","a"))
print(classification_report(test_y, bert_predicted), file=open("output.txt", "a"))
torch.save(bert_clf,'/home/parush/bert_models/bert_mohammed')
